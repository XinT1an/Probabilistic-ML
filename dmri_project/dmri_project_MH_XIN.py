...
class frozen_prior:
    ...
    def __init__(self, alpha_S=2, theta_S=500, alpha_lambda=4, theta_lambda=2.5e-4):
    ...
    def rvs(self, size=1):
        ...
        # Sample eigenvalues (must be positive)
        if size == 1:
            evals = gamma.rvs(a=self.alpha_lambda, scale=self.theta_lambda, size=3)
        else:
            evals = gamma.rvs(a=self.alpha_lambda, scale=self.theta_lambda, size=(size, 3))
        
        # Sample random rotations (uniform on SO(3))
        if size == 1:
            evecs = Rotation.random().as_matrix()
        else:
            evecs = np.array([Rotation.random().as_matrix() for _ in range(size)])
        
        return S0, evals, evecs
        
    def logpdf(self, S0, evals, evecs):
        # Gamma prior for S0
        logp_S0 = gamma.logpdf(S0, a=self.alpha_S, scale=self.theta_S)
        
        # Gamma prior for each eigenvalue
        logp_lambda = np.sum(gamma.logpdf(evals, a=self.alpha_lambda, scale=self.theta_lambda), axis=-1)
        
        # Uniform on SO(3): constant, logpdf = 0
        return logp_S0 + logp_lambda
...
class frozen_likelihood:
    ...
    def __init__(self, gtab,y,sigma=29.0):
    ...
    def logpdf(self, S0, evecs, evals):
        ...
        qDq = np.einsum('ij, sjk, ik -> si', q, D, q)
        

        S = S0[:, None] * np.exp(-qDq)
        
        loglik = np.sum(norm.logpdf(self.y, loc=S, scale=self.sigma), axis=-1) 
